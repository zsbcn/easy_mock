...
class InterfaceService(BaseService):
    def get_interface_list(self, project_id) -> list[InterfaceResponse]:
    ...
    def create_interface(self, interface: InterfaceCreate, user_id: str) -> None:
        interface_exist: Interface = self.db.query(Interface).where(Interface.name == interface.name).first()
        if interface_exist:
            raise BusinessException(ProjectConstants.PROJECT_EXIST)
        interface_db: Interface = Interface(create_by=user_id, **interface.model_dump())
